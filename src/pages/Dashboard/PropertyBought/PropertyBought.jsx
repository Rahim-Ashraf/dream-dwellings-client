            ...
            {propertyBoughts.map(propertyBought => <div key={propertyBought._id} className="card card-compact bg-base-100 shadow-xl">
                ...
                <div className="flex justify-between gap-4 p-4">
                    ...
                    <div className="my-auto">
                        {propertyBought.verification_status === "accepted" ? <Link to={`/payment/${propertyBought._id}`}><button className="btn bg-[#0055ff] text-white">Pay</button></Link> : propertyBought.verification_status === "bought" ? <h2>transaction id: {propertyBought.transaction_id}</h2> : ""}
                    </div>
                </div>
            </div>)}
        </div>
